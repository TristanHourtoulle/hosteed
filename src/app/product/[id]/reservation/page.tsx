  ...
  if (showPayment) {
    return (
      <div className='max-w-2xl mx-auto p-4'>
        ...
        <PaymentForm amount={totalAmount} onError={error => setError(error.message)} />
      </div>
    )
  }
        ...
        {!product ? (
          <div className='text-center py-12'>
            <p className='text-red-600'>
              Impossible de charger les informations de l&apos;hébergement
            </p>
          </div>
        ) : (
              ...
              <form onSubmit={handleSubmit} className='space-y-6'>
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setFormData(prev => ({ ...prev, peopleNumber: parseInt(e.target.value) }))
                    }
              ...
              <div className='space-y-4'>
                ...
                <div className='border-t pt-4'>
                  <div className='flex justify-between font-medium'>
                    <span>Total</span>
                    <span>{calculateTotalPrice().toString()}€</span>
                  </div>
                </div>
                <div className='border-t pt-4'>
                  ...
                  <div className='space-y-2'>
                    <div className='flex justify-between text-sm'>
                      ...
                      <span>
                        +
                        {(
                          ((BigInt(product.basePrice) * BigInt(calculateNights()) +
                            product.options
                              .filter(option => selectedOptions.includes(option.id))
                              .reduce((sum, option) => sum + option.price, BigInt(0))) *
                            BigInt(product.commission)) /
                          BigInt(100)
                        ).toString()}
                        €
                      </span>
