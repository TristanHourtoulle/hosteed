...
import { getRentById, changeRentStatus, approveRent, RentWithDates } from '@/lib/services/rents.service';
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                ...
                {rent.status === RentStatus.CHECKIN && rent.payment === PaymentStatus.CLIENT_PAID && (
                  <button
                ...
                {rent.status === RentStatus.CHECKOUT && rent.payment === PaymentStatus.MID_TRANSFER_DONE && (
                  <button
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">{rent.product?.user?.[0]?.name || 'Non spécifié'}</p>
                    <p className="text-sm text-gray-500">{rent.product?.user?.[0]?.email || 'Non spécifié'}</p>
                  </div>
                  <div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-gray-900">{prices?.totalPricesPayable.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Prix disponible</p>
                    <p className="text-gray-900">{prices?.availablePrice.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Prix en attente</p>
                    <p className="text-gray-900">{prices?.pendingPrice.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</p>
                  </div>
                  <div>
