...
export default function SEOFieldsCard({ seoData, onSeoChange, articleTitle }: SEOFieldsCardProps) {
  ...
  const getMetaTitleStatus = () => {
    ...
    if (length < 30)
      return {
        ...
        message: 'Trop court (30-60 caractères recommandés)',
      }
    if (length > 60)
      return {
        status: 'long',
        color: 'text-red-600',
        message: 'Trop long (60 caractères max recommandés)',
      }
  ...
  const getMetaDescriptionStatus = () => {
    ...
    if (length < 120)
      return {
        status: 'short',
        color: 'text-orange-600',
        message: 'Trop courte (120-160 caractères recommandés)',
      }
    if (length > 160)
      return {
        status: 'long',
        color: 'text-red-600',
        message: 'Trop longue (160 caractères max recommandés)',
      }
    return { status: 'good', color: 'text-green-600', message: 'Longueur optimale' }
  }
  ...
  const SearchPreview = ({ device }: { device: 'desktop' | 'mobile' }) => (
    <div
      className={`border rounded-lg p-4 bg-white ${device === 'mobile' ? 'max-w-sm' : 'max-w-xl'}`}
    >
      <div className='space-y-1'>
        <h3
          className={`text-blue-600 hover:underline cursor-pointer ${device === 'mobile' ? 'text-sm' : 'text-lg'}`}
        >
        ...
        <p
          className={`text-slate-600 ${device === 'mobile' ? 'text-xs' : 'text-sm'} leading-relaxed`}
        >
        ...
        >
          <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              />
              <p className={`text-xs ${titleStatus.color}`}>{titleStatus.message}</p>
            </div>
            ...
            <div className='space-y-2'>
              ...
              <Textarea
                ...
                rows={3}
              />
              <p className={`text-xs ${descriptionStatus.color}`}>{descriptionStatus.message}</p>
            </div>
