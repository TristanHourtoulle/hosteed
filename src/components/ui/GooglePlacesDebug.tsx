  ...
  const testPlacesAPI = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/places/autocomplete?input=${encodeURIComponent(input)}&types=(cities)&language=fr`
      )
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Erreur ${response.status}: ${errorData.error || errorData.details || 'Erreur inconnue'}`
        )
      }
    ...
    <div className='p-4 bg-yellow-50 border border-yellow-200 rounded-lg'>
      ...
      <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              {results.map((result, index) => (
                <div key={index} className='p-2 bg-white rounded text-sm'>
                  <div>
                    <strong>Ville:</strong> {result.structured_formatting?.main_text}
                  </div>
                  <div>
                    <strong>Pays:</strong> {result.structured_formatting?.secondary_text}
                  </div>
                  <div>
        ...
        <div className='text-xs text-gray-600 p-3 bg-gray-100 rounded'>
          <p>
            <strong>Variable d&apos;environnement:</strong>{' '}
            {process.env.NEXT_PUBLIC_GOOGLE_PLACES_API ? 'Configurée' : 'Non configurée'}
          </p>
          <p>
            <strong>Note:</strong> Cette variable est utilisée côté serveur via les routes API
          </p>
          <p>
            <strong>Variable utilisée:</strong> NEXT_PUBLIC_GOOGLE_PLACES_API
          </p>
        </div>
      </div>
    </div>
