...
export async function getPayablePricesPerRent(rentId: string): Promise<PayablePrices> {
  try {
    ...
    } else if (rent.status === RentStatus.CHECKIN && (rent.payment === PaymentStatus.CLIENT_PAID || rent.payment === PaymentStatus.MID_TRANSFER_DONE)) {
...
export async function createPayRequest(rentId: string, type: PaymentStatus, userId: string, notes: string, method: PaymentMethod) {
  try {
    const rent = await prisma.rent.findUnique({
      where: {id: rentId},
      include: {
    ...
    if (type == PaymentStatus.FULL_TRANSFER_REQ && (rent.status == RentStatus.CHECKOUT || rent.product.contract )) {
    ...
    } else if (type == PaymentStatus.MID_TRANSFER_REQ && (rent.status == RentStatus.CHECKIN || rent.product.contract )) {
      ...
      if (!payrequest) throw new Error('Internal Error during the payment requests');
    } else if (type == PaymentStatus.REST_TRANSFER_REQ && rent.payment == PaymentStatus.MID_TRANSFER_DONE) {
      const tempPrices = price / 2;
      const payrequest = await prisma.payRequest.create({
        data: {
          user: { connect: {id: userId}},
