...
export class GoogleSuggestionService {
  ...
  async getCitySuggestions(options: GoogleSuggestionOptions): Promise<GooglePlacePrediction[]> {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/autocomplete?${params.toString()}`,
        {
          method: 'GET',
          headers: {
  ...
  async getPlaceDetails(options: GooglePlaceDetailsOptions): Promise<GooglePlaceDetails | null> {
    try {
      const { placeId, fields = ['place_id', 'name', 'formatted_address', 'geometry', 'address_components', 'types'], sessionToken } = options;
  ...
  async getCitySuggestionsByCountry(input: string, countryCode: string): Promise<GooglePlacePrediction[]> {
