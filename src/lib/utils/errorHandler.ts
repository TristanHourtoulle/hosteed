...
export function parseCreateProductError(error: unknown): ErrorDetails {
  ...
  if (
    ...
    errorMessage.includes('Aucun utilisateur assigné')
  ) {
    return {
      type: 'auth',
  ...
  if (
    ...
    return {
      ...
      details: [
        ...
        errorMessage.includes('taille') || errorMessage.includes('size')
  ...
  if (
    errorMessage.includes('hôtel') ||
    errorMessage.includes('hotel') ||
    errorMessage.includes('chambres')
  ) {
    return {
      type: 'validation',
